...
namespace SCSI
{
    class SCSICommandParser
    {
        private static uint ReadUInt24(byte[] buffer, int offset)
        {
        ...
        public static uint GetCDBTransferLength(byte[] commandBytes, uint deviceType)
        {
            switch ((SCSIOpCodeName)commandBytes[0])
            {
                case SCSIOpCodeName.RecoverBufferedData:           // DATA_IN (2-4)
                    return ReadUInt24(commandBytes, 2);
                ...
                case SCSIOpCodeName.ReadPosition:                  // DATA_IN (7-8) 
                    if (deviceType == SPTITarget.TAPE_DEVICE)
                    {
                ...
                case SCSIOpCodeName.ReadBuffer10:                  // DATA_IN (6-8)
                    return ReadUInt24(commandBytes, 6);
        ...
        public static uint GetDiskReadTransferLength(byte[] commandBytes, uint blockSize)
        {
        ...
        public static uint GetTapeReadTransferLength(byte[] commandBytes, uint blockSize)
        {
            uint transferLength = 0;
            ...
            switch ((SCSIOpCodeName)commandBytes[0])
            {
                ...
                case SCSIOpCodeName.ReadReverse16:                 // DATA_IN (12-14)
                    transferLength = ReadUInt24(commandBytes, 12);
                    break;
                case SCSIOpCodeName.Read6:                         // DATA_IN (2-4)
                case SCSIOpCodeName.ReadReverse6:                  // DATA_IN (2-4)
                    transferLength = ReadUInt24(commandBytes, 2);
                    break;
                default:
                    throw new NotSupportedException("Invalid CDB when parsing READ Transfer Length");
            }
